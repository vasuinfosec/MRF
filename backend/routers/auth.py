...
from server import (
    ...
    ROLES, LEGACY_ROLE_MAP, _ensure_roles_shape, permission_roles,
)
...
async def list_users(authorization: Optional[str] = Header(None)):
    u = await get_current_user(authorization)
    is_admin = "admin" in permission_roles(u)
    if not is_admin:
        # Non-admin: return only lightweight directory info (name/role) without emails
        users = await db.users.find({}, {"_id": 0}).to_list(1000)
        return [UserOut(**{**_ensure_roles_shape(x), "email": ""}) for x in users]
    users = await db.users.find({}, {"_id": 0}).to_list(1000)
    return [UserOut(**_ensure_roles_shape(u2)) for u2 in users]


@api.post("/users/role", response_model=UserOut)
async def set_role(body: RoleUpdate, authorization: Optional[str] = Header(None)):
    # V2 has a roles[]-authoritative endpoint with stricter Director policy.
    # Leaving this scalar endpoint live would bypass those checks.
    if os.environ.get("ACCESS_SECURITY_V2", "0") == "1":
        raise HTTPException(404, "Not found")
    u = await get_current_user(authorization)
    if u.role != "admin":
