...
def permission_roles(user) -> set[str]:
    """Return the roles that may be used for authorization.

    V2 always authorizes from ``roles``.  The scalar ``role`` is only used to
    backfill genuinely old records in ``_ensure_roles_shape`` and remains the
    source of truth while V2 is disabled.
    """
    if isinstance(user, dict):
        shaped = _ensure_roles_shape(dict(user))
        role = shaped.get("role") or ""
        roles = shaped.get("roles") or []
    else:
...
def require_roles(*allowed):
    async def dep(user: UserOut = None, authorization: Optional[str] = Header(None)):
        ...
        roles = permission_roles(u)
        if not (roles & (set(allowed) | {"admin"})):
            raise HTTPException(status_code=403, detail=f"Roles {sorted(roles)} not allowed")
        return u
    return dep
...
async def startup():
    ...
    # ---- Task 4: category-linked material definitions and approved UOMs ----
    # Sparse composite key preserves all legacy material rows while providing
    # database-level duplicate protection for Task 4 records.
    await db.materials.create_index("material_key", unique=True, sparse=True)
    await db.material_categories.create_index("category_id", unique=True)
    await db.material_categories.create_index("name_norm", unique=True)
    await db.uoms.create_index("uom_id", unique=True)
    await db.uoms.create_index("code_norm", unique=True)
...
from routers import material_master as _material_master_router  # noqa: E402,F401
