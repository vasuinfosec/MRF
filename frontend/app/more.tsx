...
export default function More() {
  ...
  const load = async () => {
    try {
      const [u, v, p] = await Promise.all([api<any[]>("/users"), api<any[]>("/vendors"), api<any[]>("/projects")]);
        ...
        <View style={{ marginTop: 8 }}>
          {projects.map((p) => (
            <View key={p.project_id} style={styles.row}>
              <View><Text style={{ fontWeight: "700" }}>{p.code}</Text><Text style={{ color: theme.colors.textMuted, fontSize: 12 }}>{p.name} · {p.site}</Text></View>
            </View>
          ))}
        </View>
      </Card>
