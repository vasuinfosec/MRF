...
export default function More() {
  ...
  const [accessConsoleEnabled, setAccessConsoleEnabled] = useState(false);

  const authRoles = user?.roles?.length ? user.roles : (user?.role ? [user.role] : []);
  const canManageAccess = !!user?.is_active && authRoles.some((r) => r === "admin" || r === "director");

  useEffect(() => {
    load();
    if (canManageAccess) {
      api("/admin/access/permissions/me")
        .then(() => setAccessConsoleEnabled(true))
        .catch(() => setAccessConsoleEnabled(false));
    }
    ...
    <AppShell title="More" testID="more-screen">
      ...
      <Card style={{ marginTop: 12 }} testID="reports-card">
        ...
        <View style={{ marginTop: 8, gap: 8 }}>
          ...
          {accessConsoleEnabled ? (
            <Btn
              testID="access-console-btn"
              title="Admin Access Console"
              variant="outline"
              icon={<Ionicons name="shield-checkmark-outline" size={18} color={theme.colors.text} />}
              onPress={() => router.push("/access-console")}
            />
          ) : null}
